...
export function parseReleaseTagCliArgv(
  argv: string[],
  versionForHelp: string,
): ParseReleaseTagCliResult {
  let dryRun = false;
...
export async function tagAndPushRelease(options: ReleaseTagOptions): Promise<ReleaseTagResult> {
  ...
  if (!allowDirty) {
    ...
    if (status.stdout.trim()) {
      throw new ReleaseTagError(
        "Working tree has uncommitted changes. Commit or stash first, or pass --allow-dirty.",
      );
    }
  }
