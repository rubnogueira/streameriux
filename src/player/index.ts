...
type AudioEngine = {
  ...
  playBuffer: (channels: Float32Array[], timestamp: number, mediaStart: number, wallStart: number) => void
  stopQueued: () => void
}
 ...
 */
export function computeUnixOffset(live: boolean, unix: boolean, end: number, nowMs = Date.now()): number | null {
  if (!live) return null
  if (unix) return 0
...
export class StreamPlayer {
  ...
  private nativeSink: ((pixels: Uint8Array, width: number, height: number, stride: number) => void) | null = null
   ...
   */
  setNativeSink(sink: ((pixels: Uint8Array, width: number, height: number, stride: number) => void) | null): void {
  ...
  async open(
    ...
    try {
      ...
      let videoTrack: InputVideoTrack | null = videoTracks[0] ?? (await input.getPrimaryVideoTrack())
      ...
      const tracks = [videoTrack, audioTrack].filter((track): track is NonNullable<typeof track> => track !== null)
      this.tracks = tracks

      const liveFlags = await Promise.all(tracks.map((track) => track.isLive()))
      const live = liveFlags.some(Boolean)
      const unix = (await Promise.all(tracks.map((track) => track.isRelativeToUnixEpoch()))).some(Boolean)
      ...
      let playAt = first
      if (live) {
        const intervals = (await Promise.all(tracks.map((track) => track.getLiveRefreshInterval()))).filter(
    ...
    const poll = async () => {
      ...
      try {
        ...
        if (stillLive.every((value) => !value)) {
          this.emit({ live: false, seekable: computeSeekable(false, first, nextEnd), unixOffset: null })
          return
        }
      } catch {
  ...
  async play(): Promise<void> {
    ...
    if (!this.state.live && this.clock.now() >= this.state.end && this.state.end > this.state.start) {
  ...
  private async runAudio(generation: number): Promise<void> {
    ...
    try {
      for await (const sample of iterator) {
        ...
        this.audio.playBuffer(channels, timestamp, this.clock.mediaTime, this.clock.audioWallOrigin)
