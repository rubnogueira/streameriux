...
import { syncEpg, urlsToSync, type EpgSyncProgress } from "./sync";
...
export function useEpg(
  ...
  const syncTick = useWallClock(
    ...
    config.enabled && epgBackgroundSyncEnabled(),
  );
  if (syncTick !== syncTickRef.current) {
    syncTickRef.current = syncTick;
    if (config.enabled && epgBackgroundSyncEnabled()) void runSync(false);
  }
