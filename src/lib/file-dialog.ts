...
async function readSpawnStdout(spawn: DialogSpawn, command: string[]): Promise<string> {
...
export async function pickCatalogFile(deps: FileDialogDeps): Promise<string | null> {
  if (deps.platform === "darwin") {
    const text = await readSpawnStdout(deps.spawn, [
      "osascript",
      "-e",
  ...
  const text = await readSpawnStdout(deps.spawn, ["powershell", "-NoProfile", "-Command", script]);
  return text || null;
}

/** Native "choose folder" dialog for the base folder holding catalogs/playlists. */
export async function pickCatalogFolder(deps: FileDialogDeps): Promise<string | null> {
  if (deps.platform === "darwin") {
    const text = await readSpawnStdout(deps.spawn, [
      "osascript",
      "-e",
  ...
  if (deps.platform === "linux") {
    const proc = deps.spawn(
      [
        ...
        "--file-selection",
        "--directory",
        "--title=Choose the folder for your playlists and channels",
      ],
      { stdout: "pipe", stderr: "pipe" },
    );
    const text = proc.stdout ? (await new Response(proc.stdout).text()).trim() : "";
...
export function pickCatalogFileNative(): Promise<string | null> {
  return pickCatalogFile(defaultDeps());
}

export function pickCatalogFolderNative(): Promise<string | null> {
  return pickCatalogFolder(defaultDeps());
}
