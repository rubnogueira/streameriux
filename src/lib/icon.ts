...
function extensionFor(url: string, contentType: string | null): string {
  ...
  if (fromUrl && (EXTS as readonly string[]).includes(fromUrl)) return fromUrl === 'jpeg' ? 'jpg' : fromUrl
  return 'png'
}
...
export async function cacheRemoteIcon(url: string): Promise<string | undefined> {
  ...
  const task = (async () => {
    try {
      ...
      const path = join(dir, `${cacheKey(url)}.${extensionFor(url, response.headers.get('content-type'))}`)
