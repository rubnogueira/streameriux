...
const OWN_PROCESS = `first process whose unix id is ${process.pid}`

/** Read the window's current native fullscreen state, or null if unknown. */
export async function isNativeFullscreen(): Promise<boolean | null> {
  if (process.platform !== 'darwin') return null
  const value = await output('osascript', [
    '-e',
    `tell application "System Events" to tell (${OWN_PROCESS})
      try
 ...
 */
export async function setNativeFullscreen(on: boolean): Promise<boolean> {
  if (process.platform === 'darwin') {
    ...
    return (await isNativeFullscreen()) ?? on
  }

  if (process.platform === 'linux') {
    const state = on ? 'add' : 'remove'
    if (await run('wmctrl', ['-r', ':ACTIVE:', '-b', `${state},fullscreen`])) return on
    await run('xdotool', ['getactivewindow', 'key', 'F11'])
    return on
  }
