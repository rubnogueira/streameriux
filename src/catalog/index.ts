...
function userDataDir(): string {
  if (process.platform === 'darwin') return join(homedir(), 'Library', 'Application Support', 'streameriux')
  if (process.platform === 'win32') return join(process.env.APPDATA ?? join(homedir(), 'AppData', 'Roaming'), 'streameriux')
...
export async function setBaseFolder(dir: string | null): Promise<void> {
  ...
  try {
    if (existsSync(configPath())) data = parse(readFileSync(configPath(), 'utf8')) as Record<string, unknown>
  } catch {
    data = {}
  }
  if (trimmed) {
...
function looksLikeUrl(value: string): boolean {
  return /^(https?|rtmp|rtsp):\/\//i.test(value) || value.endsWith('.m3u8') || value.endsWith('.m3u') || value.endsWith('.mpd')
}
...
function optional(channel: Channel, key: keyof Channel, rec: Record<string, unknown>, ...aliases: string[]): void {
...
export function channelFromUnknown(
  ...
  const name = asString(rec.name) ?? asString(rec.title) ?? asString(rec['tvg-name']) ?? asString(rec.tvg_name) ?? fallbackId
  const id = asString(rec.id) ?? asString(rec['tvg-id']) ?? asString(rec.tvg_id) ?? slug(fallbackId)
  ...
  optional(channel, 'group', rec, 'group-title', 'group_title', 'group', 'tvg-group', 'tvg_group', 'category')
  ...
  optional(channel, 'userAgent', rec, 'user_agent', 'user-agent', 'http_user_agent', 'http-user-agent')
  optional(channel, 'referrer', rec, 'referrer', 'referer', 'http_referrer', 'http_referer', 'http-referrer')
  ...
  optional(channel, 'catchupDays', rec, 'catchup-days', 'catchup_days', 'catchup-back', 'catchup_back')
  optional(channel, 'timeshift', rec, 'timeshift')
  if (rec.radio === true || asString(rec.radio) === 'true' || asString(rec.radio) === '1') channel.radio = true
  const iconRaw = asString(rec.icon) ?? asString(rec.logo) ?? asString(rec.tvg_logo) ?? asString(rec['tvg-logo'])
  if (iconRaw) {
    const icon = usableIconSrc(resolveIconPath(iconRaw, sourceFile) ?? (looksLikeRemote(iconRaw) ? iconRaw : undefined))
...
async function readHidden(): Promise<string[]> {
  ...
  return Array.isArray(data.urls) ? data.urls.filter((value): value is string => typeof value === 'string') : []
}
...
export function orderedGroupNames(
  ...
  const set = new Set(channels.map((channel) => resolveGroupKey(channel, usePlaylistGroups, assignments)))
...
function parseAny(text: string, sourceFile: string, editable: boolean): Channel[] {
  if (isM3uPlaylistText(text) || sourceFile.toLowerCase().endsWith('.m3u') || sourceFile.toLowerCase().endsWith('.m3u8')) {
...
export async function loadCatalog(options: { fresh?: boolean } = {}): Promise<Catalog> {
  ...
  const groups = orderedGroupNames(channels, usePlaylistGroups, groupAssignments, groupsConfig.names)
...
async function writeUserChannels(channels: Channel[]): Promise<void> {
  const body =
    ...
    channels.map((channel) => channelToToml({ ...channel, sourceFile: userPath(), sourceKind: 'toml', editable: true })).join('\n\n') +
    '\n'
...
export async function removeCatalogSource(ref: string): Promise<void> {
  const list = await readSourceList()
  list.files = list.files.filter((entry) => resolveListed(entry) !== resolveListed(ref) && entry !== ref)
  list.playlists = list.playlists.filter((entry) => entry !== ref && resolveListed(entry) !== resolveListed(ref))
  await writeSourceList(list)
}

export async function upsertUserChannel(input: Partial<Channel> & { name: string; url: string }): Promise<Channel> {
  ...
  const index = channels.findIndex((channel) => channel.url === trimmedUrl || channel.id === next.id)
...
export async function removeChannel(channel: Channel): Promise<void> {
  if (channel.editable) {
    const channels = (await readUserChannels()).filter((entry) => entry.url !== channel.url && entry.id !== channel.id)
    await writeUserChannels(channels)
    return
  }
...
export function useCatalog(): Catalog & {
  ...
  addLink: (ref: string, options?: AddCatalogSourceOptions) => Promise<{ kind: 'playlist' | 'channel'; count: number }>
  ...
  const refresh = useCallback(async () => {
    ...
    setCatalog(await loadCatalog({ fresh: true }))
  }, [])
