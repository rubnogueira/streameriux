...
`,
    ...
    expect(channels).toMatchObject([{ id: 'one', name: 'One', url: 'https://example.com/one.m3u8' }])
  })
...
`,
    ...
    expect(channels).toMatchObject([{ id: 'two', name: 'Two', url: 'https://example.com/two.m3u8' }])
  })
...
`,
  ...
  it('resolves local and remote icons', () => {
    ...
    expect(resolveIconPath('https://example.com/logo.png', toml)).toBe('https://example.com/logo.png')
...
`,
  ...
  it('puts ungrouped channels in No group at the end', () => {
    const names = orderedGroupNames(
      [
        { id: 'a', name: 'A', url: 'https://a', sourceFile: 'x', sourceKind: 'toml', editable: false, group: 'United States' },
        { id: 'b', name: 'B', url: 'https://b', sourceFile: 'x', sourceKind: 'toml', editable: false },
        { id: 'c', name: 'C', url: 'https://c', sourceFile: 'x', sourceKind: 'toml', editable: false, group: 'Live' },
      ],
      true,
      {},
    )
  ...
  it('ignores playlist groups when disabled and honors assignments', async () => {
    ...
    expect(orderedGroupNames([channel], catalog.usePlaylistGroups, catalog.groupAssignments)).toEqual(['Sports'])
  })
})
...
describe('looksLikePlaylistRef', () => {
  ...
  it('detects Xtream/OTT panel URLs that have no file extension', () => {
    expect(looksLikePlaylistRef('http://host:8080/get.php?username=u&password=p&type=m3u_plus')).toBe(true)
  })
...
describe('addCatalogSource', () => {
  ...
  mkdirSync(dir, { recursive: true })
  const prev = process.env.STREAMER_CHANNELS_DIR
  ...
  afterAll(() => {
    if (prev == null) delete process.env.STREAMER_CHANNELS_DIR
    else process.env.STREAMER_CHANNELS_DIR = prev
  })
  ...
  it('does not fall back to user.toml when playlist intent gets non-playlist content', async () => {
    ...
    const user = existsSync(join(dir, 'user.toml')) ? readFileSync(join(dir, 'user.toml'), 'utf8') : ''
...
describe('orderedGroupNames', () => {
  it('includes custom group names even without channels', () => {
    const channels = [
      { id: 'a', name: 'A', url: 'https://example.com/a', sourceFile: 'x', sourceKind: 'toml' as const, editable: false },
    ]
...
describe('gpuixIconSrc', () => {
  ...
  it('returns a real file for materialized UI icons', () => {
    const path = materializeSvg('test-radio', '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 8 8"><circle cx="4" cy="4" r="3" fill="#000"/></svg>')
