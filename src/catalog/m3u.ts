...
export function parseM3uStreamUrl(raw: string): Pick<Channel, 'url' | 'userAgent' | 'referrer' | 'headers'> {
  const trimmed = raw.trim()
...
function isHlsManifest(head: string): boolean {
  return /#EXT-X-(TARGETDURATION|MEDIA-SEQUENCE|STREAM-INF|PLAYLIST-TYPE|ENDLIST|KEY|MAP|BYTERANGE)/i.test(head)
}
...
export function parseM3u(text: string, sourceFile: string): Channel[] {
  ...
  for (const raw of lines) {
    ...
    if (/^#EXTINF:/i.test(line)) {
      ...
      pending = {
        name,
        group: primaryGroup(attrs['group-title']) ?? groupOverride ?? primaryGroup(attrs['tvg-group']),
