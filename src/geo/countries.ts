import { COUNTRIES, COUNTRY_OPTIONS, REGIONS, WORLDWIDE_CODES, type CountryInfo } from './countries.data'
...
export function orderedCountryKeys(map: Map<string, unknown>, tail = NO_COUNTRY): string[] {
  const named = [...map.keys()].filter((name) => name !== tail).sort((a, b) => countrySortKey(a).localeCompare(countrySortKey(b)))
