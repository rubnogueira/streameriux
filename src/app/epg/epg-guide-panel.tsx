...
export function EpgGuidePanel({
  ...
  let body: ReactNode
  if (!epg.enabled) {
    body = <text style={{ fontSize: 13, fontFamily: FONT, color: C.tertiary }}>EPG is disabled — enable it in Settings</text>
  } else if (!epgId) {
    body = <text style={{ fontSize: 13, fontFamily: FONT, color: C.tertiary }}>No guide data for this channel</text>
  } else if (schedule.length === 0) {
    body = <text style={{ fontSize: 13, fontFamily: FONT, color: C.tertiary }}>No programmes in this window — try Sync now in Settings</text>
  } else {
    body = (
      <WindowedList
          ...
          return (
            <div
              ...
              style={{
                ...
                hover: programme.desc ? { backgroundColor: active ? C.accentSoft : C.overlay } : undefined,
              }}
            >
              <text style={{ fontSize: 10, fontFamily: FONT, color: active ? C.live : C.ghost }}>
                {formatEpgRange(programme.start, programme.stop, now)}
              </text>
              <text style={{ fontSize: 13, fontFamily: FONT, fontWeight: active ? '600' : '500', color: C.text }}>
                {programme.title}
              </text>
              {programme.category ? (
                <text style={{ fontSize: 10, fontFamily: FONT, color: C.tertiary }}>{programme.category}</text>
              ) : null}
              {open && programme.desc ? (
                <text style={{ fontSize: 11, fontFamily: FONT, color: C.secondary, marginTop: 4 }}>{programme.desc}</text>
              ) : null}
            </div>
          )
        }}
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 10 }}>
          ...
          <div style={{ flexGrow: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 2 }}>
            <text style={{ fontSize: 15, fontFamily: FONT, fontWeight: '600', color: C.text }}>{channel.name}</text>
            {epgId ? (
