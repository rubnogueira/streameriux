...
import { ActionButton, IconButton, IntervalChip, SettingToggle, TextField, useAsyncAction } from '../components/primitives'
...
export function EpgPanel({
  ...
  return (
    <div style={{ flexGrow: 1, minHeight: 0, display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      />
      <text style={{ fontSize: 11, fontFamily: FONT, fontWeight: '600', color: C.tertiary, paddingLeft: 2 }}>
        SYNC INTERVAL
      </text>
      <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 6, paddingLeft: 2 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 8, paddingLeft: 2, paddingRight: 2 }}>
        ...
        />
        <ActionButton icon="check" label="Set" testId="epg-set-interval" onClick={applyCustomInterval} />
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6, paddingLeft: 2 }}>
        ...
        <text style={{ fontSize: 11, fontFamily: FONT, color: C.secondary, marginTop: 4 }}>Hours after now</text>
        <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>
          {PRESET_GUIDE_HOURS.map((hours) => (
            <IntervalChip
              key={`after-${hours}`}
      ...
      {epg.config.customUrls.length ? (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
          ...
          {epg.config.customUrls.map((url) => (
            ...
            >
              <text style={{ flexGrow: 1, fontSize: 11, fontFamily: FONT, color: C.secondary, whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>
                {url}
              </text>
              <IconButton icon="trash" size={28} color={C.ghost} onClick={() => void epg.removeCustomUrl(url)} />
            </div>
          ))}
        </div>
      ) : null}
      {detected.length ? (
        <div style={{ flexGrow: 1, minHeight: 0, display: 'flex', flexDirection: 'column', gap: 4 }}>
          <text style={{ fontSize: 11, fontFamily: FONT, fontWeight: '600', color: C.tertiary, paddingLeft: 2 }}>
            DETECTED FROM PLAYLISTS
          </text>
          <div style={{ flexGrow: 1, minHeight: 0, overflow: 'scroll', display: 'flex', flexDirection: 'column', gap: 4 }}>
            {detected.map((source) => (
              <div
                key={source.id}
                style={{
              ...
              >
                <text style={{ fontSize: 12, fontFamily: FONT, color: C.text }}>{source.label}</text>
                <text style={{ fontSize: 10, fontFamily: FONT, color: C.ghost, whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>
                  {(source.epgUrl ?? '').slice(0, 120)}{(source.epgUrl?.length ?? 0) > 120 ? '…' : ''}
                </text>
              </div>
            ))}
          </div>
