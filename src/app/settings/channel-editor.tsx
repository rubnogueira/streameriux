...
export function EditorField({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
      <text style={{ fontSize: 11, fontFamily: FONT, fontWeight: '600', color: C.tertiary }}>{label.toUpperCase()}</text>
      {readOnly ? (
        <div
          style={{
            minHeight: 36,
        ...
        >
          <text style={{ fontSize: 12, fontFamily: FONT, color: C.secondary, whiteSpace: 'nowrap', textOverflow: 'ellipsis' }}>
            {value || '—'}
          </text>
        </div>
      ) : (
...
export function CountrySelect({ value, onChange }: { value: string; onChange: (value: string) => void }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
      <text style={{ fontSize: 11, fontFamily: FONT, fontWeight: '600', color: C.tertiary }}>COUNTRY</text>
      <div style={{ position: 'relative' }}>
        ...
        >
          <text style={{ flexGrow: 1, fontSize: 12, fontFamily: FONT, color: selected ? C.text : C.tertiary }}>
        ...
        {open ? (
          ...
          >
            ...
            {COUNTRY_OPTIONS.map((option) => (
              ...
              >
                <text style={{ fontSize: 12, fontFamily: FONT, color: C.text }}>{option.label}</text>
              </div>
            ))}
          </div>
        ) : null}
...
export type ChannelDraft = { name: string; url: string; group: string; country: string; language: string; chno: string; icon: string }
  ...
  return (
    <div style={{ flexGrow: 1, minHeight: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 8 }}>
        ...
        {onDelete ? (
          <ActionButton icon="trash" label="Remove" testId="channel-editor-delete" onClick={() => run(onDelete(), onBack)} />
        ) : null}
        <ActionButton
          icon="check"
      ...
      <div style={{ flexGrow: 1, minHeight: 0, overflow: 'scroll', display: 'flex', flexDirection: 'column', gap: 12 }}>
        <EditorField label="Name" value={draft.name} placeholder="Channel name" readOnly={groupOnly} onChange={(v) => set({ name: v })} />
        <EditorField
          label="Stream URL"
          value={draft.url}
        ...
        <div style={{ display: 'flex', flexDirection: 'row', gap: 12 }}>
          <div style={{ flexGrow: 1 }}>
            <EditorField label="Category / group" value={draft.group} placeholder="e.g. Sports" onChange={(v) => set({ group: v })} />
          </div>
          {groupOnly ? null : (
            <div style={{ width: 140, flexShrink: 0 }}>
              <EditorField label="Channel no." value={draft.chno} placeholder="12" onChange={(v) => set({ chno: v })} />
            </div>
          )}
        </div>
        {groups.length ? (
          <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>
            {groups.slice(0, 40).map((name) => (
              <GroupChip key={name} label={name} active={draft.group === name} onClick={() => set({ group: name })} />
            ))}
          </div>
        ) : null}
        {groupOnly ? null : (
          <>
            <div style={{ display: 'flex', flexDirection: 'row', gap: 12 }}>
              ...
              <div style={{ flexGrow: 1 }}>
                <EditorField label="Language" value={draft.language} placeholder="e.g. Portuguese" onChange={(v) => set({ language: v })} />
              </div>
            </div>
            <EditorField label="Icon URL or path" value={draft.icon} placeholder="https://…/logo.png" onChange={(v) => set({ icon: v })} />
          </>
        )}
