  ...
  return (
    <div style={{ flexGrow: 1, minHeight: 0, display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        <text style={{ fontSize: 13, fontFamily: FONT, fontWeight: '600', color: C.text, paddingLeft: 2 }}>
          Base folder
        </text>
        <text style={{ fontSize: 12, fontFamily: FONT, color: C.secondary, paddingLeft: 2 }}>
          Where your playlists, channels, and cache are stored. Changing it reloads the catalog from the new location.
        </text>
        <div
          style={{
            display: 'flex',
        ...
        >
          <text
            ...
            style={{ flexGrow: 1, minWidth: 0, fontSize: 12, fontFamily: FONT, color: baseFolder ? C.text : C.tertiary }}
          >
            {baseFolder ?? `Default · ${resolvedBaseFolder}`}
          </text>
          <ActionButton icon="folderPlus" label="Choose…" testId="base-folder-choose" onClick={chooseFolder} />
          {baseFolder ? (
            <ActionButton
              icon="refresh"
              label="Reset"
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {SIDEBAR_VIEWS.map((entry) => {
          ...
          return (
            ...
            >
              <text style={{ flexGrow: 1, fontSize: 13, fontFamily: FONT, fontWeight: active ? '600' : '500', color: C.text }}>
                {entry.label}
              </text>
