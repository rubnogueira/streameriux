...
export function AddChip({ label, onClick, testId }: { label: string; onClick: () => void; testId: string }) {
  return (
    <div
      testId={testId}
      onClick={onClick}
...
export function Overlay({ children, onMouseEnter }: { children: ReactNode; onMouseEnter?: () => void }) {
  return (
    <div
...
export function ActionButton({
  ...
  return (
    ...
    >
      ...
      <text style={{ fontSize: 12, fontFamily: FONT, fontWeight: '600', color: primary ? C.onAccent : C.text }}>
        {label}
      </text>
    </div>
  )
...
export function GroupChip({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      style={{
    ...
    >
      <text style={{ fontSize: 11, fontFamily: FONT, color: active ? C.live : C.secondary }}>{label}</text>
    </div>
  )
}
  ...
  return (
    ...
    >
      <div style={{ flexGrow: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 2 }}>
        <text style={{ fontSize: 13, fontFamily: FONT, fontWeight: '500', color: C.text }}>{label}</text>
        {hint ? <text style={{ fontSize: 11, fontFamily: FONT, color: C.ghost }}>{hint}</text> : null}
      </div>
      <div
        style={{
          width: 36,
...
export function IntervalChip({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <div
      onClick={onClick}
      style={{
...
export function SectionHeader({ label, count }: { label: string; count: number }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', gap: 8, minHeight: 32, paddingLeft: 10, paddingRight: 10, marginTop: 6 }}>
      <text style={{ flexGrow: 1, fontSize: 11, fontFamily: FONT, fontWeight: '600', color: C.tertiary }}>{label.toUpperCase()}</text>
