...
import { pickCatalogFile, pickCatalogFolder } from "../lib/file-dialog"
import { isNativeFullscreen, setNativeFullscreen } from "../lib/fullscreen"
...
export function StreamerApp() {
  ...
  const [nativeFullscreen, setNativeFullscreenState] = useState(false)
  ...
  const nativeChannelUp = !!selected && playerState.status !== 'idle' && playerState.status !== 'error'
  ...
  const step = (delta: number) => {
    ...
    const index = Math.max(0, list.findIndex((channel) => channel.id === current))
    const next = list[(index + delta + list.length) % list.length]
    if (next) select(next)
  }
  stepRef.current = step

  if (!catalog.loading && selectedId && !visibleChannelList.some((channel) => channel.id === selectedId)) {
  ...
  const enterFullscreen = () => {
    ...
    void setNativeFullscreen(true).then((ok) => setNativeFullscreenState(ok))
  }
  ...
  if (fullscreen && IS_MAC && nativeFullscreenPoll !== lastNativePollRef.current) {
    ...
    void isNativeFullscreen().then((native) => {
    ...
    >
      ...
      {dialog === 'settings' ? (
        <SettingsDialog
          ...
          onPickBaseFolder={pickCatalogFolder}
          onSetBaseFolder={catalog.setBaseFolder}
          onClose={() => setDialog(null)}
          onAddLink={(reference) => catalog.addLink(reference, { intent: 'playlist' }).then(() => undefined)}
          onDeleteSource={(source) => catalog.deleteSource(source.id.replace(/^(file|playlist):/, ''))}
