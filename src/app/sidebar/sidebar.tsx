...
import { ChannelMark, ChannelNumber, ChannelRow, GroupRow } from '../components/channel'
...
export function ViewTabs({ view, onView }: { view: SidebarView; onView: (view: SidebarView) => void }) {
  return (
    <div
      style={{
        display: 'flex',
...
export const Sidebar = memo(function Sidebar({
  ...
  const groupFor = (channel: Channel) => resolveGroupKey(channel, usePlaylistGroups, groupAssignments)
  ...
  const navNames = useMemo(() => {
    ...
    return view === 'countries' ? orderedCountryKeys(navMap, navTail) : orderedKeys(navMap, navTail)
  ...
  return (
    ...
    >
      ...
      >
        ...
        {showBack ? (
          ...
          >
            ...
            <text
              style={{ flexGrow: 1, minWidth: 0, fontSize: 12, fontFamily: FONT, fontWeight: '600', color: C.text }}
            >
              {searching ? 'Search results' : view === 'countries' && drill ? countryLabel(drill) : drill}
            </text>
            <text style={{ fontSize: 11, fontFamily: FONT, color: C.ghost }}>{String(channelList.length)}</text>
          </div>
        ) : (
      ...
      {showNav ? (
        ...
        <div style={{ flexGrow: 1, minHeight: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', paddingLeft: 24, paddingRight: 24 }}>
          <text style={{ fontSize: 13, fontFamily: FONT, color: C.tertiary, textAlign: 'center' }}>{emptyChannelsLabel}</text>
        </div>
      ) : (
        <WindowedList
      ...
      >
        <div style={{ flexGrow: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 3 }}>
          {catalogError ? (
            <text style={{ fontSize: 12, fontFamily: FONT, color: C.accent }}>Some sources failed to load</text>
          ) : null}
          {epgSyncLabel ? (
            <text
              testId="epg-sync-status"
              style={{ fontSize: 11, fontFamily: FONT, color: epgSyncLabel.startsWith('Syncing') ? C.secondary : C.ghost }}
            >
              {epgSyncLabel}
            </text>
          ) : null}
        </div>
        <IconButton icon="refresh" testId="refresh-catalog" onClick={onRefresh} color={C.secondary} />
        <IconButton icon="settings" testId="open-settings" onClick={onSettings} color={catalogError ? C.accent : C.secondary} />
      </div>
    </div>
  )
})
