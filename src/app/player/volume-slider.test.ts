...
describe("volume slider helpers", () => {
  it("applyVolumePointer ignores missing ratios", () => {
    ...
    applyVolumePointer(eventPayload(), () => null, { current: null }, onChange);
  ...
  it("applyVolumePointer maps pointer position to volume", () => {
    ...
    applyVolumePointer(eventPayload({ x: 25 }), () => bounds, { current: bounds }, onChange);
  ...
  it("finishVolumePointerDrag ends active drags", () => {
    ...
    const apply = vi.fn();
    finishVolumePointerDrag(dragging, apply, eventPayload({ x: 1 }));
    expect(apply).not.toHaveBeenCalled();

    dragging.current = true;
    dragRouter.current = { move: vi.fn(), end: vi.fn() };
    finishVolumePointerDrag(dragging, apply, eventPayload({ x: 10 }));
