...
export const EMPTY_FRAME_LAYERS: FrameLayers = {
  a: null,
  b: null,
  aGen: 0,
  bGen: 0,
...
export function pushFrameLayers(snapshot: FrameLayers, path: string | null): FrameLayers {
  if (!path) return EMPTY_FRAME_LAYERS;
  const back = snapshot.front === "a" ? "b" : "a";
  const genKey = back === "a" ? "aGen" : "bGen";
  return {
    ...snapshot,
    [back]: path,
    [genKey]: snapshot[genKey] + 1,
    front: back,
  };
}
...
export function usePlayerFrameLayers(player: StreamPlayer | null): FrameLayers {
  const storeRef = useRef({
    ...
    push(path: string | null) {
      this.snapshot = pushFrameLayers(this.snapshot, path);
    },
  });
